function saveToFile() {
        ...
        function highlightSubstringsWithTooltip(textareaId, color, substrIndexes, tooltipText) {
            const textarea = document.getElementById(textareaId);

            if (!textarea || !color || !substrIndexes || !substrIndexes.length) {
                console.error('Invalid input. Please check your parameters.');
                return;
            }

            const text = textarea.value;
            const highlightedText = substrIndexes.map(([start, end]) =>
                `<span style="background-color: ${color};" title="${tooltipText}">${text.substring(start, end)}</span>`
            );

            const newText = substrIndexes.reduce((acc, [start, end], index) => {
                const before = acc + text.substring(acc.length, start);
